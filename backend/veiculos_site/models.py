        ...
        if self.status == 'aprovado' and self.motorista_designado and self.veiculo_designado:
            ...
            if conflitos_motorista.exists():
                raise ValidationError(
                    f"Conflito de agendamento: O motorista {self.motorista_designado} já está ocupado neste horário."
                )
            ...
            if conflitos_veiculo.exists():
                raise ValidationError(
                    f"Conflito de agendamento: O veículo {self.veiculo_designado} já está ocupado neste horário."
                )

    def save(self, *args, **kwargs):
        is_new = self.pk is None
        status_antigo = None if is_new else Chamado.objects.get(pk=self.pk).status
